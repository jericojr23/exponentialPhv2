...
function TasksPageSideBarForm({
  ...
  const handleRemoteOkChange = (checked) => {
    console.log(checked);
    //TODO: send request and filter tasks
    setSideBarFormState((prevState) => {
      return { ...prevState, remoteOkOnly: !prevState.remoteOkOnly };
    });
  };

  const handleFeaturedTasksOnlyChange = (checked) => {
    console.log(checked);
    //TODO: send request and filter tasks
    setSideBarFormState((prevState) => {
      return { ...prevState, featuredTasksOnly: !prevState.featuredTasksOnly };
    });
  };

  const handleTaskTypeSelect = (e, option) => {
    console.log(e.target.checked, option);
    if (e.target.checked) {
      setSideBarFormState((prevState) => {
        const taskTypes = [...prevState.taskTypes];
        taskTypes.push(option);
        return { ...prevState, taskTypes };
      });
    } else {
      setSideBarFormState((prevState) => {
        return {
          ...prevState,
          taskTypes: prevState.taskTypes.filter((taskType) => option != taskType),
        };
      });
    }
  };
    ...
    <div className='space-y-8'>
      ...
      <div className='bg-white shadow-lg rounded-sm border border-slate-200 p-5'>
        <div className='grid md:grid-cols-2 xl:grid-cols-1 gap-6'>
          ...
          <Switch.Group as='div' className='flex items-center'>
            <Switch
              checked={sideBarFormState.featuredTasksOnly}
              onChange={handleFeaturedTasksOnlyChange}
              className={classNames(
                sideBarFormState.featuredTasksOnly
                  ? 'bg-indigo-600'
                  : 'bg-gray-200',
                'relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500'
              )}
                  ...
                  <li key={option.value}>
                    <label className='flex items-center'>
                      <input
                        ...
                        onChange={(e) => handleTaskTypeSelect(e, option.value)}
                        checked={sideBarFormState.taskTypes.includes(
                          option.value
                        )}
                      />
      ...
      <div className='relative bg-indigo-200 rounded-sm p-5 min-w-60'>
        ...
        <div className='relative'>
          <div className='text-sm font-medium text-slate-800 mb-2'>
            Remember to keep track of your task research.
          </div>
...
export default TasksPageSideBarForm;